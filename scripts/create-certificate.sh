...
# Path to the custom Homely $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.homely.$(hostname).cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.homely.$(hostname).crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.homely.$(hostname).key"
...
BASE_CNF="
    [ ca ]
    default_ca = ca_homely_$(hostname)
...
then
    ...
    # Finally, generate the private key and certificate signed with the Homesly $(hostname) Root CA.
